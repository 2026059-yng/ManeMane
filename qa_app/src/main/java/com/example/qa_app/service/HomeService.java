...
public class HomeService extends BaseService{
  ...
  //実支出額を計算
  public int calcActualExpenses(int user_id){
    return homeRepository.findExpensesThisMonth(user_id)
              - homeRepository.findExtraIncThisMonth(user_id);
  }

  //現在の使用可能金額の計算
  public int calcAvailableBalance(int user_id){
    return super.calcMonthlyBudget(user_id)
              - calcActualExpenses(user_id);
  }

  //現在の使用可能金額・収入・固定費合計・実支出をListに格納し、返す
  public List<Integer> returnAmounts(int user_id){
