...
public class BaseService {
  ...
  // 収入を表示
  public int showIncome(long user_id) {
    return homeRepository.findIncome(user_id);
  }

  // カテゴリーを表示
  public List<Category> showCategories(long user_id) {
    return homeRepository.findCategories(user_id);
  }

  // 使用可能金額の予算を計算（収入 - 固定費合計）
  public int calcMonthlyBudget(long user_id) {
    return homeRepository.findIncome(user_id) - calcSumFixedCosts(user_id);
  }

  // 固定費の合計を計算
  public int calcSumFixedCosts(long user_id) {
    int fixedCosts = 0;
    List<Monthly> costs = homeRepository.findFixedCosts(user_id);

    for (Monthly monthly : costs) {
      fixedCosts += monthly.getMonthlyAmount();
    }
